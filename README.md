...
- POS 点餐收银（`pos.html`）：跑堂 PIN、菜号点单、送厨、桌号锁、外带取餐号、一起结 / 分开结、跑堂结算、日结、DEP 交易日志，见 `docs/POS.md`
...
管理台第一次打开时让你设一个密码，之后改菜单就输这个密码。密码存在你自己的后端
（D1 或本机 SQLite）里，存的是 PBKDF2 哈希，不是密码本身；登录换回来的是一个有效
12 小时的会话令牌。改密码会让所有已登录的设备重新输一次。

需要注意的一点：网站是公开的，谁先打开管理台谁就能设这个密码。上线后请立刻设上。

忘了密码，用 `ADMIN_TOKEN` 重设：在「连接设置」里填上令牌，再到「管理密码」里改。
令牌本身也还能直接当经理用——墙上那种配一次就不再动的平板还是用令牌。

## 角色与操作记录

除了管理密码，还有三个共享令牌，各自一个角色（在「连接设置」里填）：
...
收银在 POS（`http://<服务器>:8787/pos.html`）：管理台添加跑堂和 PIN，平板用经理密码配对后，跑堂登录点单、送厨、结账。
