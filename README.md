...
- 顾客：邮箱账户、收藏菜品、菜单直接加购物车下单（堂食扫码 / 外带自取，直接出单，带时段、开台、份数金额、下单间隔等限制）、积分和兑换，见 `docs/GUESTS.md`
...
- POS（跑堂 / 前台）：`http://127.0.0.1:8787/pos.html`

POS 和管理台各自可以装成单独的 App（浏览器菜单里「安装应用 / 添加到主屏幕」），打开直接进入，不经过顾客菜单。
